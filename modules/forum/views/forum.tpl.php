	...
	<tbody class="forum-content">
		...
		<tr>
			...
			<td class="col-6">
				...
				<div><?php echo icon('fas fa-user').' '.($topic['user_id'] ? $this->user->link($topic['user_id'], $topic['username']) : '<i>'.$this->lang('Visitor').'</i>').' '.icon('far fa-clock').' '.time_span($topic['date']) ?></div>
			</td>
			<td class="col-2">
			...
			<td class="col-3">
				...
				<div><small><?php echo icon('fas fa-user').' '.($topic['last_user_id'] ? $this->user->link($topic['last_user_id'], $topic['last_username']) : '<i>'.$this->lang('Visitor').'</i>').' '.icon('far fa-clock').' '.time_span($topic['last_message_date']) ?></small></div>
				<?php else: ?>
		...
		<?php if (empty($topics)): ?>
		<tr>
			<td colspan="4"><div class="alert alert-info text-center mb-0"><?php echo $this->lang('No message') ?></div></td>
		</tr>
		<?php endif ?>
	</tbody>
</table>
